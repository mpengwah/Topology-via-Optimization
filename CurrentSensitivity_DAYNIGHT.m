...
for phase = 1:3
    switch(phase)
        ...
        case(2)%phase B
            fprintf('Phase B\n');
            [scriptvar.r,scriptvar.c] = find(data.Ib==0);
            scriptvar.edges = unique(scriptvar.r);
            scriptvar.counts = histc(scriptvar.r,scriptvar.edges);
            scriptvar.row = scriptvar.edges.*(scriptvar.counts>37870/2);
            scriptvar.row(scriptvar.row==0)=[];
        ...
        case(3)
            fprintf('Phase C\n');
            [scriptvar.r,scriptvar.c] = find(data.Ic==0);
            scriptvar.edges = unique(scriptvar.r);
            scriptvar.counts = histc(scriptvar.r,scriptvar.edges);
            scriptvar.row = scriptvar.edges.*(scriptvar.counts>37870/2);
            scriptvar.row(scriptvar.row==0)=[];
